...
1. title: SEO-friendly Japanese title ~32 chars that includes series + chapter + "?????????????".
...
4. spoiler: {
     ...
     "foreshadowings": 3-4 bullet strings focusing on???????????,
     "predictions": 2 bullet strings for ????? with explicit reasoning and "??" wording.
   }
5. glossary_updates: 1-3 items { "term": "...", "reading": optional, "description": "?????????????1-3?" }.
   - Prefer new terms/abilities/?/?? from this chapter; if none, lightly enrich an existing term.
6. reference_links: only from provided list. Each item { "label": "...", "url": "..." }.

Return JSON only.
""")
ARTICLE_SPOILER_REVIEW_TMPL = """
??JSON????????????????????????synopsis?1500???????????????JSON???????????
...
Return JSON only.
""")
ARTICLE_INSIGHT_REVIEW_TMPL = """??JSON?????????????????????????????????JSON???????????
